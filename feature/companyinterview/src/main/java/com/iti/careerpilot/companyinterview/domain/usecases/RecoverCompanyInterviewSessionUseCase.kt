...
class RecoverCompanyInterviewSessionUseCase @Inject constructor(
    private val repository: CompanyInterviewRepository
) {
    suspend operator fun invoke(token: String): CareerPilotResult<CompanyInterviewSessionState, NetworkError> {
        return repository.recoverSession(token)
    }
}
