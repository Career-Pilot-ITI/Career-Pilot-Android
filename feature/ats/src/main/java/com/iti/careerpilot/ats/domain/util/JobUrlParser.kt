...
object JobUrlParser {
    ...
    fun isValidHttpsUrl(value: String): Boolean {
        ...
        if (!LINKEDIN_JOB_REGEX.matches(candidate)) return false
    ...
    private fun String.isLoopbackAddress(): Boolean =
        this == IPV6_LOOPBACK ||
                startsWith(IPV4_LOOPBACK_PREFIX)
    ...
    private val HTTPS_CANDIDATE = Regex(
        """https://[^\s<>"]+""",
        RegexOption.IGNORE_CASE
    )
    private val LINKEDIN_JOB_REGEX = Regex(
