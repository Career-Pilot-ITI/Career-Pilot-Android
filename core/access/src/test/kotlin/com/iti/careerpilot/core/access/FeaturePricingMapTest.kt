...
class FeaturePricingMapTest {
    ...
    fun `coinCost returns exact backend prices for all known features`() {
        assertEquals(5, FeaturePricingMap.coinCost(FeatureKey.MockInterviews))
        assertEquals(5, FeaturePricingMap.coinCost(FeatureKey.VoicePracticeMode))
        ...
        assertEquals(2, FeaturePricingMap.coinCost(FeatureKey.AtsFeatures))
        assertEquals(5, FeaturePricingMap.coinCost(FeatureKey.CvAiAnalysis))
        assertEquals(2, FeaturePricingMap.coinCost(FeatureKey.CoverLetter))
    ...
    fun `allPrices map contains correct mappings`() {
        ...
        assertEquals(5, prices[FeatureKey.MockInterviews])
        assertEquals(5, prices[FeatureKey.VoicePracticeMode])
        assertEquals(15, prices[FeatureKey.VideoInterview])
        assertEquals(10, prices[FeatureKey.Quizzes])
        assertEquals(2, prices[FeatureKey.AtsFeatures])
        assertEquals(5, prices[FeatureKey.CvAiAnalysis])
        assertEquals(2, prices[FeatureKey.CoverLetter])
