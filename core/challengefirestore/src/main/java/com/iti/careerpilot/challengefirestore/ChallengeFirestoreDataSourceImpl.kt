...
class ChallengeFirestoreDataSourceImpl @Inject constructor(
    ...
    override suspend fun deleteChallenge(challengeId: String, visibility: ChallengeVisibility): CareerPilotResult<Unit, FirebaseError> = safeFirebaseCall {
        val collection = getCollectionName(visibility)
        firestore.collection(collection).document(challengeId).delete().await()
    }

    override suspend fun getCreatedChallenges(creatorId: Long): CareerPilotResult<List<Challenge>, FirebaseError> = safeFirebaseCall {
    ...
    override suspend fun getTakenChallenges(participantId: Long): CareerPilotResult<List<ChallengeSession>, FirebaseError> = safeFirebaseCall {
    ...
    override suspend fun getChallengeSessions(challengeId: String): CareerPilotResult<List<ChallengeSession>, FirebaseError> = safeFirebaseCall {
        firestore.collection(FirestoreCollections.CHALLENGE_SESSIONS)
